...
public class ChatSocket extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception { System.out.println("session = " + session);
        ...
        System.out.println("접속명단.size() = " + 접속명단.size());
    }

    // 2. 클라이언트 소켓과 접속 종료되었을때 ( session : 접속이 종료된 클라이언트 소켓 정보 )
    @Override
    ...
    // 3. 클라이언트 으로부터 메시지를 받았을떄( session : 메시지를 보낸 클라이언틔 소켓 , message : 클라이언트가 받은 메시지   )
    @Override
